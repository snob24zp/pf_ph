...
class Data_Show2:
    def Data_show(self,df_p,df_n,DR):
...
# =============================================================================
        ...
        print("df_combined.columns)",list(df_combined.columns))
...
#line for phases        
        ...
        segment_lengths = [(name, int(round(seconds * DR.Data_Points_Per_Second))) for name, seconds in periods]
...
class Some_Processor:
    def del_pause(self,df,phase,DR):
        periods = [
            ("Baseline", DR.Baseline),
            ("Absorb", DR.Absorb),
            ("Pause", DR.Pause),
        ...
        df_cleaned = df.drop(df.columns[to_drop], axis=1)
        return df_cleaned
...
if __name__=='__main__':
    ...
    folder_pass_path = "./p"
    folder_fail_path = "./n"
    dfp,_=DR.read_result(folder_pass_path)
    dfn,_=DR.read_result(folder_fail_path)
    dfp= SP.del_pause(dfp,"Pause",DR)
    dfn= SP.del_pause(dfn,"Pause",DR)
    DSh.Data_show(dfp,dfn,DR)